...
- **팀 배포**(2026-07-23~): Docker 배포 정본은 전용 repo [stonefish_bringup](https://github.com/HERO-Lab-POSTECH/stonefish_bringup)(멀티스테이지, core+sim+slam 소스 bake — 호스트 실빌드 검증 완료). `.hq/config/project/env/`는 그 동기화 미러(구 bind-mount 키트·repo `docker/` 사본은 제거됨). 협업 규칙은 워크스페이스 `CONTRIBUTING.md`, 실험 분석 프로파일은 `.hq/config/experiments/profile/`. 워크스페이스 루트가 meta-repo(git)로 관리됨.
