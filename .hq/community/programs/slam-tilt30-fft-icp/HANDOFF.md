...
`nssm.min_st_sep` **양방향** — base(15) · sep8(느슨) · sep30(빡빡) 3팔 인터리브
3블록 9 런(`sep.txt`, 15:31 착수, ~17:12 종료 예상). base 팔의 `min_pcm` 은 4 다.

근거는 `loopdiag.py` 실측(k1·k3 의 base/pcm4 4 런): 후보 수는 팔과 무관하게 321~325 로
같고(**PCM 은 후보 생성이 아니라 채택만 바꾼다**), 키프레임 간격 0-30 구간이 노름비
0.765~0.785 · 전진축 오차 중앙 −2.20~−3.65 m 로 전 구간 최악이며 전체 후보의 14% 다.
그 하한 15 가 `nssm.min_st_sep` 값 그 자체다(`localization.py:490` 의
`target_frames = range(current_key - min_st_sep)`).

dot 경로는 복사본에 `yaml_set.py … nssm.min_st_sep=30` 을 걸어 exit 0 으로 사전 검증했다.
