...
`min_pcm=5` 로 **벽 위치**를 좁힌다 — `m*b4`(override 없음 = 새 기본값 4) 대
`m*pcm5` 2팔 인터리브 3블록 6 런(`pcm5.txt`, 13:14 착수, ~14:21 종료 예상).

구조적 상한이 6 근처다. `_trim_nssm_queue` 의 창은 *개수 상한이 아니라 키프레임
인덱스 폭*이라(`current_key - queue[0].source_key > pcm_queue_size`, 5) 큐는 키
k−5..k 의 후보를 담아 최대 6 개다. 따라서 `min_pcm=6` 은 전건 일치를 요구해
사실상 도달 불가이고 5 가 마지막으로 의미 있는 값이다. 더 밀려면
`pcm_queue_size` 를 같이 올려야 하는데 그건 **다음 변수다**(한 번에 하나).

**부수 확인**: `m*b4` 는 override 없이 도는 팔이므로, 이 팔이 옛 base 대역
(2.9~3.6)이 아니라 pcm4 대역(2.2~2.6)으로 나와야 커밋된 기본값이 실제로 먹었다는
증거가 된다. 옛 대역으로 나오면 리빌드가 config 를 안 집은 것이다.

## 대기 중인 축 — `nssm.min_st_sep` (양방향, 미실행)

`loopdiag.py` 를 k1·k3 의 base/pcm4 4 런에 걸어 얻은 관측이다. 후보 수는 팔과
무관하게 321~325 로 사실상 같다 — **PCM 은 후보 생성을 안 바꾸고 채택만 바꾼다.**

키프레임 간격 0-30 구간이 전 구간에서 최악이다:

| 간격 | n | 시드/GT 노름비 | ex 중앙 (전진축) |
|:--|--:|:--|:--|
| **0-30** | ~45 | **0.765~0.785** | **−2.20 ~ −3.65 m** |
| 30-80 | ~100 | 0.897~0.930 | −0.28 ~ +0.31 |
| 80-160 | ~65 | 0.810~0.860 | −1.44 ~ −0.41 |
| 160+ | ~110 | 0.855~0.881 | −0.22 ~ +0.46 |

이 구간의 하한 15 는 `nssm.min_st_sep: 15`(최신 키프레임 주변 배제대) 그 자체다.
즉 **배제대 바로 바깥의 후보들이 전진축으로 2~3.6 m 틀린 채 PCM 에 들어간다.**
전체 후보의 14% 다.

⚠️ 이 표를 팔 비교로 읽지 말 것 — 오차는 GT 상대포즈 대비인데 후보 쌍 자체가
현재 그래프 추정으로 골라지므로, 정확한 런이 더 좋은 쌍을 고른다. 팔 간 차이는
결과이지 원인이 아니다. 구간 간 대비만 유효하다.

실행할 때는 **반드시 양방향**(예: 8 느슨 · 30 빡빡 · base 15). `finding/048` 이
바로 그 이유로 성립했다.
