...
## `min_pcm` 축 — 검정 완료, **4 채택** (`finding/048`)

양방향(2=느슨 · 4=빡빡 · base 3) 3팔 인터리브 9 런(`pcmsweep.txt`). **pcm4 가
2D mean 과 ATE 두 축 모두에서 완전분리**로 이겼다 — 2D 최대 2.577 < base 최소
2.896, ATE 최대 2.536 < base 최소 2.735. 느슨한 pcm2 는 어느 축에서도 분리가
없어 기각.
...
게이트 회피가 아니다 — `reject_pos` 가 base 16·12·14 대 pcm4 13·13·13 으로
불변이고(0.25 m / 10° 게이트는 손대지 않았다), DR 의존은 오히려 개선
(`seed_dr` base 3·2·3 → 1·2·1). 9 런 전부 유효대역, ICP 수렴 1.000.

**상관이 가리킨 반대 방향이 이겼다.** `finding/047` 의 상관은 `pcm_accepted`↑ →
`len_ratio`↑ → ATE↓ 였고 pcm2 는 실제로 채택 수를 300~304 로 올렸는데 무효였다.
채택 수를 225~258 로 **줄인** pcm4 가 이겼다. 한 방향만 걸었으면
`finding/043`·`046` 에 이어 세 번째 같은 오진이었다 — **양방향은 비용이 아니라
판별력이다.**

적용: slam `5eeb9ca` 로 기본값 3→4, 세 곳 동시(`config/slam.yaml:109` ·
`core/factor_graph.py:49` · `core/slam.py:128`). 183 passed, push 완료.
기전은 **미확정** — 버려진 루프가 평균적으로 해로웠다는 것까지이고 어느
변위대·어느 오차부호였는지는 `loopdiag.py` 로 다음에 본다.

## `min_pcm=5` — 검정 완료, 기각 (`finding/049`)

인터리브 **6 블록 12 런**(`pcm5.txt` 3 + `pcm5b.txt` 3). base 팔은 override 없이
도는 새 기본값 4 다. **두 오차축 모두 분리 없음** — 2D b4 [2.365, 2.845] 대 pcm5
[2.145, 3.166], ATE b4 [2.201, 2.728] 대 pcm5 [2.196, 2.669]. 짝지음은 두 축 모두
pcm5 4 승 2 패. 기본값은 **4 유지**.

**앞 3 블록의 "2D 완전분리" 는 산포였다.** 그 시점 수치는 pcm5 최대 2.267 < b4 최소
2.453, 짝지음 3/3 이었고, 4 블록째(b4 2.365 대 pcm5 2.579)에 곧바로 깨졌다.
`finding/048` 이 **두 축 동시** 분리였던 것이 우연이 아니라 판별 조건이었다는 뜻이다 —
단일 축 완전분리는 n=3 에서 산포와 구별되지 않는다.

`min_pcm` 축은 이것으로 닫는다. 6 은 구조적으로 도달 불가(`_trim_nssm_queue` 의 창이
개수가 아니라 키프레임 인덱스 폭이라 큐 최대 6, 전건 일치 요구). 더 밀려면
`pcm_queue_size` 를 같이 올려야 하는데 별개 변수이고, "채택 수를 22% 더 줄여도 오차가
안 따라왔다"(b4 228~253 → pcm5 181~219)는 관측이 그쪽 기대값을 낮춘다.

## `min_st_sep` 축 — 검정 완료, **양방향 기각** (`finding/050`)

`nssm.min_st_sep` 을 15 에서 양쪽으로 민 3블록 9런 인터리브(`sep.txt`, 블록마다
base→sep8→sep30, 15:31~17:10). **두 팔 모두 기각, 기본값 15 유지.**
...
- **sep8** 두 축 모두 겹침(짝지음 2승 1패, 중앙값은 축마다 갈림) → 무효.
- **sep30** ATE 가 나쁜 쪽으로 완전분리(최소 3.601 > base 최대 3.514), 짝지음 두 축
  모두 base 3승 0패 → 악화로 기각.

**예측이 뒤집혔다.** sep30 은 `loopdiag.py` 가 노름비 0.765~0.785 로 전 구간 최악이라
지목한 키프레임 간격 15–30 을 정확히 도려내는 설정이었다. `finding/041`·`finding/043`
에 이은 **세 번째 독립 재현** — 눈에 띄게 틀려 보이는 양을 옳은 방향으로 밀면 나빠진다.

기전은 개수가 아니라 구간이다: sep30 은 `pcm` 이 base 보다 몇 개 적을 뿐인데 `len_r`
0.8689~0.8697 로 전 런 최저이고, sep8 은 루프를 13% 더 채택하고도(263~269) 오차가
base 와 구별되지 않는다.

양방향 검정으로 닫은 네 번째 축이다(`043`·`046`·`048`·`050`).

## ⚠️ base 표류 — 큐를 가로지르는 비교 금지 (`finding/051`)

`finding/050` 판정 중 발견. 동일 소스·설정·오버라이드로 3.5시간에 걸쳐 돈 base 런 9개의
`len_r` 이 시각 순으로 내려가고, 앞 6런 [0.9165, 0.9323] 과 뒤 3런 [0.8913, 0.9076] 이
**완전분리**한다(p = 1/84). 소스·설정·부하·디스크·처리량·키프레임 수를 전부 배제했고
**계측하는 어느 카운터도 설명하지 못한다** — 원인 미상.

- **다른 큐의 숫자를 한 대역에 모으지 마라.** 산포 측정도, 팔 비교도 안 된다.
- `finding/045` 의 시간 교란이 실측됐다 — 인터리브는 예방이 아니라 **필수 조건**.
- 같은 설정 9런 폭이 2D [2.365, 3.244] · ATE [2.201, 3.514] 다. 이보다 작은 효과는
  인터리브 없이는 원리적으로 안 보인다. `finding/048` 의 pcm4 채택은 인터리브 짝지음에
  두 축 동시 완전분리라 유지되지만 여유는 보이던 것보다 좁다.
- 다음 프로브: 같은 설정 연속 6런으로 재현 여부(약 1시간). 재현되면 런 사이 누적물
  (`/dev/shm` · DDS 공유메모리 · 페이지 캐시 · GPU 클럭)을 런마다 찍는다.
...
- ⚠️ **`ps | grep '[r]unlist.sh'` 는 자기 자신을 잡는다.** 큐를 연쇄로 걸려고
  "다른 큐가 도는가" 가드를 넣었더니 그 가드가 **자기 래퍼 셸**을 매칭해 exit 1 로
  빠졌다(2026-09-05, sep 착수가 통째로 누락돼 머신이 40 분 놀았다). 래퍼의 argv 에
  `bash runlist.sh sep.txt` 라는 문자열이 통째로 들어 있기 때문이다 — `[r]` 대괄호
  트릭은 grep 자신만 피하지 상위 셸은 못 피한다. 같은 이유로 **launch 직후 pid 를
  패턴으로 캡처하면 래퍼 pid 를 잡아** 대기 루프가 즉시 빠진다(같은 날 두 번째).
  argv 필드를 정확 비교하라:

      ps -eo pid,args --no-headers | awk '$2=="bash" && $3=="runlist.sh" && $4=="<큐>.txt" {print $1}'
...
# 인계 2026-09-06 — 하루 50 런 분석과 다음 프로브 4건 (실행 0건, 전부 승인 대기)

⚠️ **이 절이 최신이다.** 위 2026-09-05 절의 판정은 유효하나 세 곳의 *근거* 가 아래에서 조정됐다
(048 의 2D 분리 여유·049 의 "벽은 4"·051 의 "단조 표류"). 계획 정본은 `PLAN-PHASE5.md`.

## 무엇을 했나

exp-analyze 로 어제 50 런(8 큐)을 교차 분석해 보고서를 썼고, exp-design 으로 proposal 4건을 썼다.
둘 다 독립 리뷰어(`report-reviewer`·`proposal-reviewer`) approve. **큐를 돌리지 않았다.**

- 보고서: `/workspace/experiments/slam-tilt30/day-260905/analysis/diagnose-20260906-060611/report.md`
  (+ `report.ko.md`, `manifest.json`, `review.json`). 스크래치 분석 스크립트는 세션 scratchpad 의
  `day_analysis.py` — 재현하려면 보고서 §A~§F 의 계산 설명대로 `metrics.json`·`[INSTR] scale` 을 읽으면 된다.
- 위키: `finding/052`(층별 분해·루프 천장) · `053`(시드 부족분 포화형) · `054`(min_pcm pooled·pcm5 검정력·4번째 쌍),
  그리고 `036`·`048`·`049`·`051` 에 2026-09-06 코멘트.
- proposal: `/workspace/experiments/slam-tilt30/day-260905/proposals/next-20260906-0623{22,23,24,25}.md`
  = D1 · F1 · L1 · L2. `omx campaign-plan-add` 로 intent 기록.

## 확정 4건 (report §3·§4·§5·§6)

1. **산포는 전부 그래프층.** 다리 걸음 FFT 시드/GT 0.9198–0.9253(50 런 상수), ICP +0.8 %, 체인 곱
   0.928 = NSSM-off 0.9292. 루프를 켠 50 런은 전부 그 아래 — **루프는 궤적을 늘린 적이 없다.**
   루프 쪽 변경의 천장은 len_r 0.928. 계열 안에서 어떤 카운터도 len_r 을 설명 못 한다.
2. **시드 부족분은 다리 8 %, 회전 1 %** — 0.007 → 0.085 → 0.10 m @ 0.5/1.05/3.17 m 로 **포화형**.
   덧셈 상수(036)도 비율도 아니다. 전진축만 편향(76 % 음수), 횡은 무편향. 이 bag 은 횡이동
   구간이 없어 온라인 로그로는 "거리 방향 대 이동 방향" 을 못 가른다.
3. **min_pcm 은 개수가 아니라 부류(추론).** 계열 중앙값 pcm2→5 단조(pooled ρ −0.62), 계열 안 ρ≈0.
   **049 의 pcm5 기각은 6 쌍으로 검정력 부족**(중앙 −0.33/−0.25 m, 4/6 승, 필요 13–19 쌍).
   048 의 2D 완전분리는 vizbase/vizpcm4 4번째 쌍(+0.276)에서 깨지고 ATE·len_r 은 4/4 유지 — 채택 불변.
4. **051 표류는 14 런으로 보면 블록 단위 수준 이동**(ρ −0.49, p 0.075). 원인 미상 그대로.

## 다음에 돌릴 것 (순서대로, 사람 승인 후)

| 순 | label | 무엇 | 비용 | 가르는 것 |
|:--|:--|:--|:--|:--|
| 1 | D1 | 동일 설정 연속 6 런 + 런 전 머신 상태 스냅샷 (`drift.txt`) | 70 min | 블록 상태 vs 단조 표류 |
| 2 | F1 | 오프라인 전진 변위 스윕 0.25–3 m, 회전 0 (`offline/disp_sweep.py` 신규) | 20–40 min, 재생 없음 | 포화형(정지 구조) vs 비례형(비평탄 바닥) |
| 3 | L1 | `[INSTR] pcm` 후보별 clique·accepted 로깅 패치 + base 3 런 | 35 min + 패치 | clique-3 루프가 압축 확정 부류인가 |
| 4 | L2 | min_pcm 5, 8 블록 추가 → 14 쌍 (`pcm5c.txt`) | 3 h | pcm5 실효(≥11/14) vs 벽은 4 |

**비평탄 바닥 h(θ)** 는 F1 이 비례형(H2)일 때 연다 — 지금 데이터는 포화형(정지 구조) 쪽이다.

## 사람 결정 (실행 전)

1. **L2 의 판정 규칙 변경** — n=14 에서 "두 축 부호검정 ≥11/14 + 중앙 개선 ≤ −0.15 m + seed_dr 비악화".
   n=3 완전분리 규칙의 이 팔 한정 대체. 확인 없이는 돌리지 않는다.
2. **프로파일** `.hq/config/experiments/profile/metrics.yaml` 은 `pending_approval: true` 이고 어휘(`ate_rmse` 등)가
   이 프로그램 지표(2D mean·ATE_RMSE·len_ratio)와 다르다. 이번 분석은 그 게이트를 넘어 진행했고
   `report-coverage` 는 그래서 vacuous pass 다. 승인·어휘 갱신 여부는 사람 결정.
3. **커밋** — `.hq/` 변경(finding 052–054·코멘트 4건·INDEX·PLAN-PHASE5·이 절·`produced-reports.jsonl`)은 미커밋.
   관행대로 `docs(hq): …` 로 올리려면 지시 필요.

## 이번 세션에 당한 것 (하네스)

- `omx wiki capture-session` 이 영문 자동 slug 스텁 10건(052–061)을 추적 스토어에 만들었다 — 걷어내고 큐레이션 3건으로 대체.
- `omx wiki add` 는 같은 제목이어도 **병합하지 않고 새 포스트를 만든다**(중복 4건 생성 → 제거). 기존 포스트
  갱신은 `hq comment <id> --author <괄호·쉼표 없음> --text`.
- `omx wiki add --category technique` 은 enum 에 없다(pattern/decision). `OmxPaths` run_id 는 `_` 시작 불가 →
  의사-런 `day-260905`. `report-review` 는 `[FINDING]`→`[EVIDENCE]`→`[CONFIDENCE: HIGH|MED|LOW]` 연속만 허용,
  `proposal-lint` 는 Diagnosis 각 레인에 `[EVIDENCE]` 필수. 리뷰어 Agent 는 라우팅 훅에 막힐 수 있다(`ROUTING_OK:`).
- `hq lint` 의 기존 오류: `finding/043` status `superseded-by-044` 가 enum 밖(이번 세션 이전부터).

# 실행 2026-09-06 — 4 축 실행 착수 (사람 승인 완료)

사용자가 "새로 세운 계획을 가지고 실험 진행해줘" 로 D1→F1→L1→L2 실행을 승인했고,
**L2 의 판정 규칙 변경은 별도 질문으로 승인**받았다(`finding/054` 코멘트에 사전등록).
어제 만든 proposal 4건은 이로써 pending approval 을 벗어났다.

## 이번에 만든 하네스 (전부 읽기 전용 / 동작 불변)

| 파일 | 무엇 | 상태 |
|:--|:--|:--|
| `snap.sh` | 런 직전 머신 상태(shm 파일 수·mem·cpu MHz·load·GPU 클럭/온도·ros 프로세스·uptime) | 새로 만듦 |
| `runlist.sh` | `SNAP=1` 일 때만 위를 `<큐>.snap.log` 에 남기는 2 줄 추가 | 기본 off, 다른 큐 동작 불변 |
| `driftdiag.py` | D1 판정기 — 런 결과와 스냅샷을 순서로 잇고 사전등록 H1/H2 임계로 찍는다 | 새로 만듦 |
| `offline/disp_sweep.py` | F1 — bag 을 프레임 층위로 읽어 직선 구간 회전 0 쌍을 변위대별로 만들고 채택 FFT 를 돌린다 | 새로 만듦, 쌍 선택 로직 합성 자체검사 통과 |
| `loopdiag.py` | L1 — `[INSTR] pcm` 을 `[INSTR] loop` 와 `(src,tgt)` 로 이어 clique 부류표를 찍는다 | 확장, 패치 이전 런에서는 안내만 |
| `drift.txt`·`pcmlog.txt`·`pcm5c.txt` | D1·L1·L2 큐 | 새로 만듦 |
| scratchpad `apply_l1_patch.py` | L1 계측 패치(factor_graph 4곳·slam.py 1곳). 큐 도는 중 적용 금지 | 미적용 |

## 코드를 읽어 해소된 것 — L1 의 측정 레인

`[INSTR] counters` 의 `pcm_accepted` 는 실제로 `factor_graph.pcm_inserted_count` 다
(`slam.py:1497` → `factor_graph.py:323`). `verify_pcm` 이 돌려준 인덱스는 아직 안 들어간
것만 삽입되므로 **고유 accepted == inserted 이고 둘은 구조상 같은 수다.** 그래서 L1 의
레인 3(“accepted 와 inserted 가 다를 수 있다”)은 런 없이 닫힌다 — 별도 카운터를 넣지 않고
후보별 clique 로깅만 남겼다. 다만 이름이 실제와 어긋나 있다는 점은 남는다.

`verify_pcm` 은 **극대 클리크 중 가장 큰 하나만** 돌려준다. 그래서 patch 는 후보별로
"그 후보를 담은 가장 큰 극대 클리크의 크기"(`clique`)와 "실제로 반환됐는가"(`accepted`)를
따로 남긴다 — 크기 4 짜리 클리크에 있어도 최대가 아니면 채택되지 않기 때문이다.

## D1 완료 — 잡음 바닥이 판정 규칙을 재라벨한다 (2026-09-06 11:06)

`drift.txt` 6 런 종료. **사전등록 H1(계단형)·H2(단조 표류) 모두 불성립**이고,
관측된 것은 제3의 구조 — **폭은 크고(len_r 0.0366) 시간 순서는 없다(rho +0.086,
p 0.872)**. 스냅샷 9 필드 중 len_r 과 붙는 것 없음(최대 `mem_used_mb` rho −0.657,
p 0.156). 기억 없는 런 간 확률변동이다. → `finding/055`

동일 설정 잡음 바닥: **2D sd 0.828 m(CV 27.2 %, 폭 2.218) · ATE sd 0.547(CV 19.2 %)
· len_r sd 0.0121(CV 1.3 %)**. 축 사이 2D~ATE r +0.706, ATE~len_r ρ −0.714,
2D~len_r +0.029.

이 잡음에서 **n=3 두 축 완전분리 규칙**(20 만 회 MC, 관측 sd·상관):

| | 값 |
|:--|--:|
| 단일 축 완전분리 오탐률 | 9.98 % (= 2/C(6,3), 데이터 무관 상수) |
| 두 축 동시·같은 방향 오탐률 | 3.04 % |
| 검정력 @ 0.598 m (048 실측 효과, 0.72 sd) | 8.5 % |
| 검정력 @ 0.30 m | 3.9 % |

귀결 셋. (1) 12 축 시도이므로 **채택 1 건이 오탐일 확률 31.0 %**. (2) 12 축이 전부
0.6 m 실효과였어도 기대 채택 수는 **1.0 건** — 관측 1/12 와 같아서, 이 성적은
'아무것도 안 통한다'와 '전부 통하는데 규칙이 못 본다'를 **구별하지 못한다**.
(3) 기각 11 축은 검정력 ≤ 8.5 % 라 거의 무정보 — `finding/054` 의 'pcm5 는 기각이
아니라 미증명'이 **닫힌 축 전체로 일반화**된다. `finding/051` 의 base 표류도 이
잡음의 한 실현으로 설명된다(comment 로 지위 하향).

**규칙을 바꿀지는 사람 결정**이며 이 세션은 바꾸지 않았다. L2 는 이미 승인된
사전등록 부호검정(14 쌍)이라 영향 없다. 한계: n=6 이라 2D sd 95 % CI
[0.517, 2.032] — 방향은 CI 하단에서도 유지되나 정확한 %는 아니다.

산출물: `snap.sh` · `driftdiag.py` · `drift.snap.log` · `d1base`~`d6base_260906_*`.

## L1 계측 패치 적용 (2026-09-06 11:14)

`exp/tilt30-localization` `1c74a7c`. `verify_pcm` 이 후보별 최대 극대 클리크 크기를
기록하고 `slam.py` 가 후보 한 줄씩 `[INSTR] pcm src= tgt= clique= accepted= inserted=`
를 방출한다. **`clique >= min_pcm` 과 `accepted` 는 다른 값이다** — `verify_pcm` 은
극대 클리크 중 가장 큰 하나만 반환한다. 계측 전용(대입·로깅뿐, `add_loop_closure` 는
`verify_pcm` 까지 조기 반환 경로 없음). 빌드·`pcmlog.txt` 3 런은 F1 종료 후.

## F1 완료 — 시드 부족분은 비례형이고, 053 의 포화형은 트리거 편향이었다 (2026-09-06 11:12)

오프라인 스윕(`offline/disp_sweep.py`, 재생 없음·6 분·실패 0). 직선 다리 18 개에서
구간당 80 쌍, 고도 중앙 6.40 m. **사전등록 H2(비례형) 성립 · H1(포화형) 불성립**
→ `finding/056`.

비율(|추정|/|참|)이 0.9038 → 0.9424 로 거의 일정한 반면, 부족분은 `d >= 1.0` 에서
0.078 → 0.116 → 0.140 → 0.188 로 **1.6 배 계속 증가**한다(H1 은 평평 + 비율 >= 0.96
을 요구했다). 횡축은 무편향 유지(ey 중앙 <= 0.029), 전진축만 편향.

적합: 비례 `0.0687·d`(SSE 9.6e−4) · 덧셈+비례 `+0.0172 + 0.0597·d`(2.9e−4) ·
포화 `0.5299·d/(d+5.468)`(3.3e−5). 포화형이 수치상 최적이지만 **k = 5.47 m 가 측정
구간 밖**이라 작동 범위에서는 포화하지 않는다. `finding/036` 의 덧셈 상수 0.06 m 도
지지되지 않는다(절편 0.017 m).

**온라인 로그(053)와의 불일치가 이번의 진짜 수확**: 두 추정은 d ≈ 1 m 에서만 일치하고
(0.085 vs 0.080) 0.5 m 에서 6.7 배, 3.17 m 에서 2.1 배 어긋난다. 1 m 는 키프레임
트리거(DR 변위 > 1.0 m)가 걸리는 지점이다 — **온라인의 포화 모양은 FFT 의 성질이
아니라 트리거가 만든 표본 편향**이다. 053·036 에 comment 로 지위 하향.

다음 축: 비례형의 기전이 (a) 비평탄 바닥 h(θ) 인지 (b) 고도·기하 척도 오차인지는
아직 안 갈린다. **`finding/041` 이 (b) 의 전역 배율 교정을 이미 기각**했으므로 이
결과를 전역 이득으로 바꾸면 안 된다. 가르려면 비율이 d 아닌 것(고도, 상관 피크의
FOV 위치)에 어떻게 의존하는지를 봐야 하며, **새 축이므로 proposal + 승인**을 거친다.

## L1 착수 (2026-09-06 11:15)

`pcmlog.txt` 3 런(`p1base`~`p3base`) 실행 중. 빌드 통과 확인. 판정은
`loopdiag.py p?base_*` 의 clique 부류 표, 행동 중립성은 `pcm_accepted` 225~258 ·
`reject_pos` 9~18 · `seed_dr` 0~5 대조로.

## L1 완료 — min_pcm 은 정밀도 필터다 (2026-09-06 11:48)

base 3 런(후보 965). 계측 행동 중립 확인(`pcm_accepted` 234/252/251, 세 지표 모두
`finding/055` 잡음대 안). → `finding/057`

**사전등록 H1·H2 둘 다 불성립** — 각 가설이 '편향'과 '크기'를 묶어 요구했는데
데이터가 그 둘을 갈랐다. 실제 구조는 **편향은 같고 산포만 다르다**: signed ex 의
Mood median test `p = 0.514`(시드 sx 중앙 −0.389 vs −0.433), `|ex|` 중앙 2.393 vs
1.128(2.12 배, Mann-Whitney `p = 2.5e−11`), IQR(ex) 4.073 vs 2.247. 네 부류 전부
단조: IQR(ex) 4.461 → 4.073 → 2.849 → 1.556 · 시드오차 4.134 → 3.460 → 2.582 → 1.357.

`finding/054` 의 '개수가 아니라 부류'는 유지되지만 부류의 정의가 '압축하는 루프' →
**'부정확한 루프'**로 정정된다.

**L2 착수 전 기록한 예측**: min_pcm 5 는 채택을 76.8 % → 25.1 %(3 배 감소)로 줄이고
정밀도 이득은 `|ex|` 1.128 → 0.905(20 %)뿐이다. NSSM-off 가 ATE 7.59 인 만큼 개수
기여가 커서 이 교환은 pcm5 에 불리해 보인다. 판정은 **승인된 사전등록 부호검정으로만**
하며 이 예측은 사후 서술을 막기 위한 기록이다.

## L2 착수 (2026-09-06 11:48)

`pcm5c.txt` 16 런(b7~b14 블록 × base4/pcm5 인터리브), 약 3 시간. 판정 규칙은
사용자 승인분: 2D·ATE 두 축 모두 14 쌍(기존 m 6 쌍 + 신규 8 쌍) 중 >= 11 승
AND 짝 중앙 개선 <= −0.15 m AND `seed_dr` 짝 중앙 <= 0.

### L2 실행 중 사전등록 (2026-09-06 11:52, 결과 관측 전)

`b7b4` 의 `feat_frames` 10500 이 정상대(D1 6 런 10830~10915 · L1 3 런 10855~10910)에서
330 벗어났다. 원인은 그 런 도중 세션이 `loopdiag` 파싱을 돌린 **CPU 경합**이다
(`finding/015` 재현). 규칙: **짝 내 두 런의 `feat_frames` 차 > 200 이면 그 짝을
confounded 로 보고 큐 끝에서 재실행해 교체한다**(제외가 아니라 교체 — n=14 유지).
임계 200 은 정상대 폭 85 의 약 2.4 배이며 `b7pcm5` 를 보기 전에 정했다.
남은 15 런 동안 세션은 CPU 작업을 돌리지 않는다.

### L2 실행 중 관측 — feat_frames 가 큐 경계에서 계단으로 내려갔다 (12:22, 원인 미상)

| 큐 | featF | kf |
|:--|:--|:--|
| D1 6 런 + L1 3 런 (10:00~11:48) | 10830~10915 | 351~352 |
| L2 b7b4·b7pcm5·b8b4 (11:48~) | 10500·10600·10580 | 346·346·349 |

9 런이 폭 85 안에 있다가 L1→L2 경계에서 일제히 약 300 내려갔다. 네 기전을 배제했다:
프로세스 잔류(= `odom_tf_bridge` 1 개, 중복 없음) · shm 누수(오래된 세그먼트 43 개로
D1 시작 09:59 때와 동일, 63 은 실행 중 세그먼트 포함) · CPU 스로틀링(2482 MHz 로 D1 의
2309~2332 보다 높다) · 메모리 압박(50.9 GB 여유). **원인 미상.**

판정에는 영향이 없다 — 인터리브라 짝 안 두 런이 같은 상태를 겪고(b7 짝 featF 차 100,
사전등록 임계 200 미만), 유효대역 `feat_frames >= 10175` 까지 여유가 325 남았다.
규칙은 바꾸지 않는다. 어떤 런이 대역 아래로 내려가면 **기존** 유효성 기준으로
무효가 되고 그 블록을 재실행한다.

`finding/055`(D1)와의 관계: D1 은 66 분 창에서 `len_r` 의 시간 순서를 못 찾아
계단형(H1)을 기각했는데, `featF` 는 더 긴 창에서 정확히 계단을 보인다. 모순이 아니라
**계단 구조가 정확도 지표가 아니라 처리량 계수에 있다**는 뜻이다.

### 정정 — feat_frames 규칙을 1차 판정에서 내린다 (12:55, 결과 관측 전)

11:52 에 세션이 추가한 `feat_frames` 짝차 200 규칙이 b8(220)·b9(290) 연속으로 걸렸다.
임계는 D1 의 featF 폭 85 에서 유도했는데 **L2 에서는 featF 산포 자체가 크다**
(6 런 10500~10915, 폭 415) — 규칙이 오염이 아니라 정상 산포를 잡고 있을 개연성이 높다.

**1차 판정은 사용자 승인 규칙(14 쌍 부호검정) 그대로**이고, featF 규칙은 민감도
분석으로 격하한다. 큐 종료 후 8 쌍의 featF 짝차 분포를 제시하고 전체 14 쌍 결과와
오염 제외 결과를 **둘 다** 보고한다. 세션이 승인 없이 만든 규칙이 승인된 설계의 n 을
줄이게 두지 않는다.

## 2026-09-06 14:46 — L2 완료, 4축 큐 전건 소진

승인된 4 proposal(D1 → F1 → L1 → L2)을 순서대로 전부 실행했다. 신규 축 채택 0 건.

| 축 | proposal | finding | 결과 |
|:--|:--|:--|:--|
| D1 | `next-20260906-062322` | 055 | 사전등록 H1·H2 둘 다 불성립. 정확도 산포는 기억 없는 잡음(2D sd 0.828 m, CV 27 %) |
| F1 | `…062323` | 056 | H2(비례형) 성립. 온라인 로그의 "포화"는 키프레임 트리거의 표본 편향 |
| L1 | `…062324` | 057 | 사전등록 H1·H2 둘 다 불성립. 클리크 크기는 편향이 아니라 **정밀도**를 가름 |
| L2 | `…062325` | 058 | **기각.** 2D·ATE 모두 10/14(필요 11, p=0.0898). `min_pcm=4` 유지 |

사전등록 가설이 4 축 중 3 축에서 불성립했다. 셋 다 "제3의 구조" 를 드러냈지
가설을 확인해 준 축은 F1 하나뿐이다.

### 프로그램 전체에 되먹임되는 두 가지

1. **채택 규칙의 검정력이 부족하다.** D1(055)이 잰 잡음 바닥에서 현행 `n=3 두 축
   완전분리` 규칙은 오탐 3.0 %·검정력 8.5 % 다. 12 축 1 채택 성적이 "전부 실효과" 와
   구별되지 않고, 기각 11 건은 거의 무정보다. L2(058)의 14 쌍 부호검정도 이 효과
   크기에서 검정력 39~54 % 라 80 % 에 26~38 쌍이 필요하다.
2. **크기를 버리는 검정이 이 도메인과 안 맞는다.** L2 데이터에 Wilcoxon 을 걸면
   두 축 다 통과한다(p=0.0148/0.0392). 효과를 큰 승리 몇 개가 나르는데 부호검정은
   −1.256 과 +0.140 을 같은 한 표로 센다. **사후 교체는 하지 않았다** — 결정은
   사람이 하고 다음 축부터 사전등록으로만 적용한다.

### 실행 중 세션이 저지른 것 (기록)

- featF "계단" 오독: D1(폭 85)·L1(폭 55)의 좁은 창을 기준선 삼아 3 런만 보고 단정.
  실제로는 어제 m 계열 폭 340 · L2 폭 415 로 넓은 산포가 평소 모습이다(055 정정).
- 그 오독에서 파생된 "짝 featF 차 200 초과 = 오염" 규칙을 승인 없이 만들었다가
  결과 관측 전 민감도 분석으로 격하. 사후 확인 결과 그 규칙을 적용해도 판정은 같고
  오히려 `seed_dr` 제약을 추가로 위반한다.
- L1 히스토그램으로 세운 "수용률 3 배 감소" 예측이 반증됨(058). 예측을 집계 전에
  적어둔 덕에 사후 서사가 아니라 반증으로 잡혔다.
- 큐 실행 중 세션이 CPU 무거운 로그 파싱을 돌려 b7b4 의 featF 를 떨어뜨렸다.
  이후 큐 종료 전까지 무거운 분석을 중단했다.

### 다음 사이클 — 사람 결정 대기

1. L2 를 26 쌍까지 연장할 것인가(12 쌍 추가 ≈ 4.5 h). 2D 만 80 % 에 닿는다.
2. 판정 검정을 크기 민감형으로 바꿀 것인가. 바꾼다면 다음 축부터 사전등록으로만.
3. 비평탄 바닥 h(θ) 축 — F1 이 H2(비례형)를 지지했으므로 열 조건이 충족됐다.
   다만 `finding/041` 이 순진한 전역 배율 보정을 이미 기각했으므로 새 proposal 이
   필요하다. 아직 작성하지 않았다.

세 항목 모두 결정 전까지 현행 설정(`min_pcm=4`)이 정본이다.

## 2026-09-06 세션 종료 — 4축 소진 뒤 시각 검수에서 드러난 것

L2 판정 뒤 사용자가 "제일 성능 좋은 결과를 rviz 로 보여달라" 고 요청했고, 그 요청이
프로그램의 두 구멍을 드러냈다. 이쪽이 오늘의 실질 수확이다.

### 구멍 1 — 기준선 없는 큐 19 런 (`finding/059`)

`queue2.txt`(12 런)·`queue4.txt`(7 런)에 기준선 팔이 **0 개**다. 큐 횡단 비교는
`finding/045`·`051` 로 무효이므로 이 19 런은 **설계상 판정 불가**다. 원인은
`runlist.sh` 가 기준선 없는 큐를 아무 경고 없이 돌린다는 것.

이 구멍 때문에 **`ssm.target_frames=5` 의 지위가 애매하다.** 같은 큐 안 네 팔 중
2D 1 위(중앙 2.141 vs 2.598~3.170)이고 이 프로그램 전체 최고 단일 런(2D 1.970)도
이 설정이 냈지만, **기본값 대비 개선은 한 번도 유효하게 측정된 적이 없다.**
세션이 처음에 "기본값 중앙 3.000 대비 0.86 m 개선" 이라고 단정했다가 스스로 반증했다 —
그 3.000 은 전 기간 62 런을 모은 값이라 큐 횡단이다.

같은 흠이 `keyframe_translation` 1.5/2.0 에도 있다. 2.0 은 DR 8.05 % 로 제약 2 위반이라
그 ATE 1.446 은 애초에 채택 대상이 아니다.

### 구멍 2 — 최고 런은 다시 볼 수 없다

`TRAJREC=1` 없이 돈 런은 궤적 좌표가 어디에도 안 남는다(로그에도 없다). 전체 1·2 위
(`n5tgt5-c` 1.970, `b13pcm5` 2.013)가 그 상태라 rviz 로 못 띄우고, 재실행해도
산포 sd 0.828 m 때문에 같은 궤적이 안 나온다. `trajrec` 는 런당 127~133 MB 이고
처리량 축을 안 건드리므로(`finding/020`) **모든 판정 런에 기본 ON 을 검토**할 만하다.

### 오늘 만든 런

| 런 | 설정 | 2D | ATE | 상태 |
|:--|:--|--:|--:|:--|
| `demo-base_260906_154704` | 기본값 | — | — | **INVALID** — 사용자 요청으로 중단 |
| `demo-best_260906_155216` | `ssm.target_frames=5` | 2.295 | 2.108 | 유효. trajrec 저장(127 MB) |

`demo-best` 로 `ssm.target_frames=5` 계열은 4 런(2D 1.970/2.141/2.543/2.295, 중앙 2.218).
계측 회계는 깨끗하다 — `factor_odom 0` · `seed_fft/seed_dr 347/3`(DR 0.86 %) ·
`icp_rate 1.0` · SSM/NSSM 전부 가동 · `reject_pos 16` 중 warp 13 구제.

### 사람 결정 대기 (4 건으로 늘었다)

1. **L2 를 26 쌍까지 연장?** 12 쌍 추가 ≈ 4.5 h. 2D 만 80 % 검정력, ATE 는 38 쌍 필요.
2. **판정 검정을 크기 민감형으로 교체?** 바꾸면 다음 축부터 사전등록으로만, L2 소급 금지.
3. **비평탄 바닥 h(θ) 축 개방?** F1 이 조건 충족. `finding/041` 이 순진한 전역 배율을
   이미 기각했으므로 새 proposal 필요 — 미작성.
4. **(신규) `ssm.target_frames=5` 를 기준선과 함께 재측정?** 이 프로그램에서 가장 유망한
   미검증 축이다. 인터리브 6 쌍이면 ≈ 2.2 h. 아울러 `runlist.sh` 에 기준선 없는 큐를
   거부하는 가드를 넣을지도 같은 결정에 딸린다(코드 변경이라 별도 승인).

넷 다 결정 전까지 `min_pcm=4` 현행 설정이 정본이다.

### 이월된 오래된 백로그 (tilt30 프로그램 밖, `needs-experiment`)

`finding/007`(velocity vs cascade 상보성) · `finding/008`(코드 건강 감사 37 건) ·
`decision/009`(Phase 2 이월 큐 N1~N13). 블로킹(`needs-apply-before-retrain`)은 0 건.
